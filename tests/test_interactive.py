...
sys.modules["calibre.ebooks.metadata.sources"] = type(sys)(
...
def save_result(
    test_type: str, query: dict, results: list, success: bool, error: str = None
):
...
class PluginTester:
    ...
    def search(
        self, title: str = None, authors: list = None, identifiers: dict = None
    ) -> list:
...
def menu_search_title_author(tester: PluginTester):
    ...
    results_data = [mi.to_dict() for mi in results]
    save_result(
        "search_title_author", {"title": title, "author": author}, results_data, True
    )
...
def menu_view_results():
    ...
    while True:
        choice = input(
            f"\nView result (1-{min(len(files), 20)}) or 0 to go back: "
        ).strip()
        if choice == "0" or not choice:
            break
        try:
